...
Example:
    >>> from nanopdf.profiler import enable_profiling, get_leak_report
...
class LeakReport:
    ...
    def __str__(self) -> str:
        ...
        lines.append("=== NanoPDF Memory Leak Report (Python) ===")
        lines.append("")
    ...
    def register_weak_ref(
        ...
        def callback(ref: weakref.ref) -> None:
            if handle in self._allocations:
                print(
                    f"[NanoPDF] Handle {handle} ({resource_type.display_name}) "
