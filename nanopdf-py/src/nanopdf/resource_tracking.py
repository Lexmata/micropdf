"""Resource Tracking and FFI Optimization for NanoPDF.
...
Example:
    >>> from nanopdf.resource_tracking import (
        ...
        def on_gc(ref: weakref.ref) -> None:
            ...
            with self._lock:
                if handle in self._resources:
                    ...
                    import os
                    if os.environ.get("NANOPDF_DEBUG"):
...
def get_object_memory_size(obj: object) -> int:
    ...
    For NanoPDF objects, this uses __sizeof__ if available,
...
def get_all_object_sizes() -> dict[str, int]:
    """Get memory sizes of all common NanoPDF types.
...
def print_memory_report(min_age_seconds: float = 60.0) -> None:
    ...
    print("=" * 60)
    print("NanoPDF Memory Report (Python)")
    print("=" * 60)
    print()
