...
func processPDFDirectory(ctx *nanopdf.Context, directory string) []PDFInfo {
	...
	for _, file := range files {
		...
		doc, err := nanopdf.OpenDocument(ctx, file)
		if err != nil {
			info.Error = err
			fmt.Printf("  ❌ Error: %v\n", err)
...
func searchInMultiplePDFs(ctx *nanopdf.Context, directory string, searchTerm string) int {
