...
func main() {
	fmt.Printf("MicroPDF Go Bindings Example\n")
...
func exampleGeometry() {
	// Point operations
	p1 := micropdf.NewPoint(100, 200)
	p2 := micropdf.NewPoint(50, 50)

	distance := p1.Distance(p2)
	fmt.Printf("Distance between points: %.2f\n", distance)

	// Rectangle operations
	rect := micropdf.NewRect(0, 0, 612, 792) // US Letter size
	fmt.Printf("Page size: %.2f x %.2f\n", rect.Width(), rect.Height())
	fmt.Printf("Contains (300, 400): %v\n", rect.ContainsXY(300, 400))

	// Matrix transformations
	fmt.Println("\nMatrix transformations:")
	point := micropdf.NewPoint(10, 20)
	fmt.Printf("Original point: (%.2f, %.2f)\n", point.X, point.Y)

	// Translate
	translated := point.Transform(micropdf.MatrixTranslate(5, 5))
	fmt.Printf("After translate(5,5): (%.2f, %.2f)\n", translated.X, translated.Y)

	// Scale
	scaled := point.Transform(micropdf.MatrixScale(2, 2))
	fmt.Printf("After scale(2,2): (%.2f, %.2f)\n", scaled.X, scaled.Y)

	// Rotate
	rotated := point.Transform(micropdf.MatrixRotate(90))
	fmt.Printf("After rotate(90°): (%.2f, %.2f)\n", rotated.X, rotated.Y)

	// Combined transformation
	combined := micropdf.MatrixScale(2, 2).Concat(micropdf.MatrixRotate(45))
...
func exampleDocument() {
	...
	if pageCount > 0 {
		...
		// Render to pixmap
		matrix := micropdf.MatrixIdentity()
