...
func TestErrorCorruptedPDF(t *testing.T) {
	...
	for _, tc := range corruptedData {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Try to open
			doc, err := micropdf.OpenDocument(ctx, tmpFile)
			if err == nil {
				doc.Drop()
...
func TestErrorNilContext(t *testing.T) {
	...
	_, err := micropdf.OpenDocument(ctx, pdfPath)
	if err == nil {
...
func TestErrorRenderingInvalidScale(t *testing.T) {
	...
	// Test with zero scale
	matrix := micropdf.MatrixScale(0, 0)
	...
	// Test with negative scale
	matrix = micropdf.MatrixScale(-1, -1)
	pix, err = page.RenderToPixmap(matrix, false)
	if err != nil {
	...
	matrix = micropdf.MatrixScale(100, 100)
	pix, err = page.RenderToPixmap(matrix, false)
	if err != nil {
