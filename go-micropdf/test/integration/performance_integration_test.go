...
func TestPerformancePageRendering(t *testing.T) {
	...
	for _, s := range scales {
		matrix := micropdf.MatrixScale(s.scale, s.scale)
		start := time.Now()
...
func TestPerformanceMultiPageDocument(t *testing.T) {
	...
	for i := 0; i < pageCount; i++ {
		...
		matrix := micropdf.MatrixScale(0.5, 0.5)
		pix, err := page.RenderToPixmap(matrix, false)
		if err != nil {
...
func TestPerformanceBufferOperations(t *testing.T) {
	...
	start := time.Now()
	for i := 0; i < iterations; i++ {
		buf := micropdf.NewBuffer(bufferSize)
		if buf == nil {
...
func TestPerformanceGeometry(t *testing.T) {
	...
	start = time.Now()
	r1 := micropdf.Rect{X0: 0, Y0: 0, X1: 100, Y1: 100}
	r2 := micropdf.Rect{X0: 50, Y0: 50, X1: 150, Y1: 150}
	p := micropdf.Point{X: 75, Y: 75}
	for i := 0; i < iterations; i++ {
		_ = r1.Union(r2)
		_ = r1.Intersect(r2)
		_ = r1.Contains(p)
