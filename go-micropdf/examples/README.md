# MicroPDF Go Examples

This directory contains practical examples demonstrating various features of the MicroPDF Go library.
...
cd ../micropdf-rs
cargo build --release
sudo make install  # Or copy to go-micropdf/lib/
```
...
doc, err := micropdf.OpenDocument(ctx, "file.pdf")
if err != nil {
    if errors.Is(err, micropdf.ErrNotFound) {
        log.Fatal("File not found")
    } else if errors.Is(err, micropdf.ErrInvalidArgument) {
...
scale := micropdf.MatrixScale(2.0, 2.0)    // 2x scale
rotate := micropdf.MatrixRotate(45)        // 45 degrees
...
```go
func processPagesConcurrently(doc *micropdf.Document) {
    pageCount, _ := doc.PageCount()
...
type PageProcessor interface {
    Process(page *micropdf.Page) error
}
...
func (te *TextExtractor) Process(page *micropdf.Page) error {
...
func processDocument(doc *micropdf.Document, processor PageProcessor) error {
    pageCount, _ := doc.PageCount()
...
**Error**: `package github.com/lexmata/micropdf/go-micropdf is not in GOROOT`
...
- [API Documentation](https://pkg.go.dev/github.com/lexmata/micropdf/go-micropdf)
