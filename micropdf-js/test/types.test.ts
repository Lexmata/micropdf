...
describe('MicroPDFError', () => {
  it('should create argument errors', () => {
    const error = MicroPDFError.argument('invalid arg');
  ...
  it('should create system errors', () => {
    ...
    const error = MicroPDFError.system('system error', cause);
    expect(error.code).toBe('SYSTEM');
    expect(error.cause).toBe(cause);
  });

  it('should create generic errors', () => {
    const error = MicroPDFError.generic('something went wrong');
    expect(error.code).toBe('GENERIC');
    expect(error.message).toBe('something went wrong');
    expect(error.name).toBe('MicroPDFError');
  });

  it('should create format errors', () => {
    const error = MicroPDFError.format('bad format');
    expect(error.code).toBe('FORMAT');
  });

  it('should create unsupported errors', () => {
    const error = MicroPDFError.unsupported('feature X');
