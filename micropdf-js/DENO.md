...
```typescript
import { Context, Document } from 'jsr:@micropdf/deno';
...
```typescript
import { Context, Document, Pixmap } from 'jsr:@micropdf/deno';
...
MicroPDF for Deno supports automatic resource cleanup using the `using` keyword (TC39 Explicit Resource Management):
...
MicroPDF for Deno provides near-native performance through:
...
| Feature      | Deno (MicroPDF) | Node.js (MicroPDF)   |
| ------------ | --------------- | -------------------- |
...
{
  "imports": {
    "@micropdf/deno": "jsr:@micropdf/deno"
  },
  "tasks": {
...
MicroPDF requires the following Deno permissions:
...
If you get "Could not find libmicropdf library":

1. Build the Rust library:

   ```bash
   cd micropdf-rs && cargo build --release
   ```

2. Verify the library exists:
   ```bash
   ls -la micropdf-rs/target/release/libmicropdf.*
   ```
...
- **Repository**: https://github.com/lexmata/micropdf
- **Documentation**: https://lexmata.github.io/micropdf/api/nodejs/
- **Rust Core**: https://docs.rs/micropdf
- **Issues**: https://github.com/lexmata/micropdf/issues
