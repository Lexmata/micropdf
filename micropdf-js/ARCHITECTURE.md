# MicroPDF Node.js Architecture

This document describes the internal architecture of the MicroPDF Node.js bindings.
...
MicroPDF for Node.js is a multi-layered architecture that provides Node.js bindings to the MuPDF PDF library via Rust FFI and C++ N-API bindings.
...
```
┌────────────────────────────────────────────────────────────┐
│  Layer 4: TypeScript API (micropdf-js/src/*.ts)            │
│  ─────────────────────────────────────────────────────────│
...
└────────────────────────┬───────────────────────────────────┘
                         ...
                         ↓
┌────────────────────────────────────────────────────────────┐
│  Layer 3: N-API Bindings (micropdf-js/native/*.cc)         │
│  ─────────────────────────────────────────────────────────│
...
└────────────────────────┬───────────────────────────────────┘
                         ...
                         ↓
┌────────────────────────────────────────────────────────────┐
│  Layer 2: Rust FFI (micropdf-rs/src/ffi/*.rs)              │
│  ─────────────────────────────────────────────────────────│
...
- `native/micropdf.cc` - Main entry point, module registration
...
- `micropdf-rs/src/ffi/mod.rs` - FFI module root
- `micropdf-rs/src/ffi/context.rs` - Context operations
- `micropdf-rs/src/ffi/document.rs` - Document operations
- `micropdf-rs/src/ffi/page.rs` - Page operations
- `micropdf-rs/src/ffi/buffer.rs` - Buffer operations
- `micropdf-rs/src/ffi/geometry.rs` - Geometry types
- `micropdf-rs/src/ffi/pixmap.rs` - Pixmap operations
- `micropdf-rs/src/ffi/text.rs` - Text operations
...
│
├── native/
│   ├── micropdf.cc         # Main N-API entry point
...
```
┌────────────────────────────────────────┐
│  TypeScript: MicroPDFError exceptions   │
└────────────────┬───────────────────────┘
                 │ try/catch
                 ↓
┌────────────────────────────────────────┐
...
```rust
// Rust FFI
pub const MICROPDF_OK: i32 = 0;
pub const MICROPDF_ERROR: i32 = -1;
pub const MICROPDF_ERROR_ARGUMENT: i32 = -2;
pub const MICROPDF_ERROR_MEMORY: i32 = -3;
pub const MICROPDF_ERROR_GENERIC: i32 = -4;
pub const MICROPDF_ERROR_IO: i32 = -10;
pub const MICROPDF_ERROR_NOT_FOUND: i32 = -11;
```
...
```python
{
  'targets': [{
    ...
    'libraries': [
      '../native/lib/<(OS)-<(target_arch)/libmicropdf.a'
    ],
