...
const GITHUB_REPO = 'lexmata/micropdf';
 ...
 */
async function main() {
  console.log(`MicroPDF v${version} installation`);
  console.log(`Platform: ${platform}-${arch}`);
  console.log('');

  // Skip if CI environment without native deps
  if (process.env.MICROPDF_SKIP_BUILD === '1') {
    console.log('MICROPDF_SKIP_BUILD=1, skipping native build');
    return;
  }

  // Check if prebuilt already exists
  const libDir = join(rootDir, 'native', 'lib', `${platform}-${arch}`);
  const libName = platform === 'win32' ? 'micropdf.lib' : 'libmicropdf.a';
