...
fn bench_colorspace_properties(c: &mut Criterion) {
    ...
    group.bench_function("n_gray", |b| {
        b.iter(|| black_box(&cs_gray).n())
    });

    group.bench_function("n_rgb", |b| {
        b.iter(|| black_box(&cs_rgb).n())
    });

    group.bench_function("n_cmyk", |b| {
        b.iter(|| black_box(&cs_cmyk).n())
    });
...
fn bench_colorspace_clone(c: &mut Criterion) {
    ...
    group.bench_function("clone_gray", |b| {
        b.iter(|| black_box(&cs_gray).clone())
    });

    group.bench_function("clone_rgb", |b| {
        b.iter(|| black_box(&cs_rgb).clone())
    });

    group.bench_function("clone_cmyk", |b| {
        b.iter(|| black_box(&cs_cmyk).clone())
    });
